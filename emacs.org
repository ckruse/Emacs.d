...
#+begin_src emacs-lisp
  ...
  (use-package mu4e
    ...
    :config (progn
              ...
              (defvar ck/has-new-messages nil)

              (defun ck/check-for-mail ()
                  (let ((retval (call-process "mu" nil nil nil "find" "(" "maildir:/Defunct/INBOX" "or" "maildir:/Termitel/INBOX" ")" "and" "flag:unread")))
                    (if (= retval 0)
                        (setq ck/has-new-messages t)
                      (setq ck/has-new-messages nil))))

              (defvar ck/mu4e-timer
                (run-at-time "1 sec" 180 'ck/check-for-mail))

              (add-to-list 'global-mode-string
                           '(:eval (when ck/has-new-messages
                                     "**Mail** ")))

              (add-hook 'mu4e-index-updated-hook 'ck/check-for-mail)
